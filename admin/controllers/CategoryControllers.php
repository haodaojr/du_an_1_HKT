...
class CategoryControllers {
    ...
    function insertcate() {
        if (isset($_POST['insert'])) {
            ...
            move_uploaded_file($tmp, "./uploads/img/" . $product_category_img);
    ...
    function updatecate($product_category_id) {
        ...
        if (isset($_POST['update'])) {
            $product_category_name = $_POST['product_category_name'];
            $product_category_img = $onecate['product_category_img']; // Initialize with existing image
            if (!empty($_FILES['product_category_img']['name'])) {
                $product_category_img = $_FILES['product_category_img']['name'];
                $tmp = $_FILES['product_category_img']['tmp_name'];
                move_uploaded_file($tmp, "./uploads/img/" . $product_category_img);
            }
            if ($this->CategoryModels->updatecate($product_category_id, $product_category_name, $product_category_img)) {
                header("Location: ?act=list_cate");
                exit;
            } else {
