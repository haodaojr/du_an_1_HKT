    ...
    public function confirmOrder($order_id) {
        try {
            ...
            if ($order && $order['order_status'] == 'completed') {
                ...
                echo "<script>alert('Đơn hàng đã được xác nhận.'); window.location.href = 'index.php?act=oderstatus';</script>";
            } else {
                echo "<script>alert('Đơn hàng không thể xác nhận vì không ở trạng thái hoàn tất.'); window.location.href = 'index.php?act=oderstatus';</script>";
            }
        } catch (PDOException $e) {
            // Rollback transaction nếu có lỗi
            $this->conn->rollBack();
